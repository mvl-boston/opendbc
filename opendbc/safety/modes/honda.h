  ...
  if (!(alternative_experience & ALT_EXP_DISABLE_STOCK_AEB)) {
    if ((bus == 2) && (addr == 0x1FA)) {
      ...
      if (honda_nidec_hybrid) {
        honda_stock_brake = (GET_BYTE(msg, 6) << 2) | (GET_BYTE(msg, 7) >> 6);
      }
      else {
        honda_stock_brake = (GET_BYTE(msg, 0) << 2) | (GET_BYTE(msg, 1) >> 6);
      }
...
static bool honda_tx_hook(const CANPacket_t *msg) {
  ...
  if ((addr == 0x1FA) && (bus == bus_pt)) {
    if ( honda_nidec_hybrid ) {
      honda_brake = (GET_BYTE(msg, 6) << 2) + ((GET_BYTE(msg, 7) >> 6) & 0x3U);
    } else {
