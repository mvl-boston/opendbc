...
static bool honda_tx_hook(const CANPacket_t *to_send) {
  ...
  int bus_buttons = (honda_bosch_radarless) ? 0 : bus_pt;  // the camera controls ACC on radarless Bosch cars / force zero for canfd
...
static safety_config honda_bosch_init(uint16_t param) {
  ...
  static CanMsg HONDA_RADARLESS_TX_MSGS[] = {{0xE4, 0, 5, true}, {0xE5, 0, 8, false}, {0x296, 0, 4, false}, {0x296, 2, 4, false}, {0x33D, 0, 8, false}};  // Bosch radarless - add bosch supplemental and zero button bus for canfd stop/go
  static CanMsg HONDA_RADARLESS_LONG_TX_MSGS[] = {{0xE4, 0, 5, true}, {0x33D, 0, 8, false}, {0x1DF, 0, 8, false}, {0x1EF, 0, 8, false}, {0x30C, 0, 8, false}, {0x18DAB0F1, 1, 8, false}};  // Bosch radarless w/ gas and brakes, swapping acc on/control and radar for canfd
...
static bool honda_bosch_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2)  {
    ...
    bool is_acc_msg = ((addr == 0x1DF) || (addr == 0x1EF) || (addr == 0x30C)) && honda_bosch_radarless && honda_bosch_long;
