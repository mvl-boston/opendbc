...
# ----------------- test override gas start -------------------
      ...
      vfactor = np.interp(CS.out.vEgo, [0.0, 0.5, 1.5, 3.0, 100.0], [1000.0, 700.0, 700.0, 50.0, 50.0])
      pcm_accel = int (np.clip(self.calc_accel * vfactor, 0, self.params.NIDEC_GAS_MAX) )
      pcm_speed = max (0.0, CS.out.vEgo + float (np.clip ( accel * 100.0 * CV.KPH_TO_MS, -9.0, +9.0 ) ) )
...
# ------------------ brake override begin
          vfactorBrake = np.interp(CS.out.vEgo, [0.0, 6.0, 100.0], [-500.0, -500.0, -120.0])
