...
from opendbc.car.honda.values import CruiseButtons, VISUAL_HUD, HONDA_BOSCH, HONDA_BOSCH_CANFD, HONDA_BOSCH_RADARLESS, \
                                     HONDA_NIDEC_ALT_PCM_ACCEL, CarControllerParams, HONDA_BOSCH_ALT_RADAR, CAR
...
#                                      k_f=1, rate= 1 / DT_CTRL / 2)
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if CC.longActive:
      ...
      if (self.CP.carFingerprint in (CAR.ACURA_MDX_3G, CAR.ACURA_MDX_3G_HYBRID, CAR.ACURA_RLX_HYBRID)) and (accel > max(0, CS.out.aEgo) + 0.1):
    ...
    elif self.CP.carFingerprint in (CAR.ACURA_MDX_3G, CAR.ACURA_MDX_3G_HYBRID, CAR.ACURA_RLX_HYBRID):
      pcm_speed_V = [0.0,
    ...
    if self.frame % 10 == 0:
      if CC.longActive and (self.CP.carFingerprint in (CAR.ACURA_MDX_3G, CAR.ACURA_MDX_3G_HYBRID, CAR.ACURA_RLX_HYBRID)):
