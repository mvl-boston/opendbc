    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          carlog.error('outstats: ' + ('bpTrue' if CS.out.brakePressed else 'bpFalse') + ' ' + ('gpTrue' if CS.out.gasPressed else 'gpFalse') + ' ' + CS.out.vEgo + ' ' \ 
                       float(self.accel) + ' ' + float(gas_pedal_force))
