  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.carFingerprint in HONDA_BOSCH:
      ...
      if (not CS.out.gasPressed) and (self.apply_brake_last == 0): # adjust speedfactor
        ...
        self.speedalpha = min(dv_sat, self.speedalpha * (1 + 0.0001 * speedfactor_error))
    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          if (CS.out.vEgo < 1e-3) and (CS.out.aEgo < 0): # restore 2m/s pid after stopped if lowspeed was stronger
