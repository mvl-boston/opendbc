...
def get_can_messages(CP, gearbox_msg):
  ...
  if CP.flags & HondaFlags.BOSCH_ALT_BRAKE_PCM_ACCEL:
    messages.append(("BRAKE_MODULE", 50))
  ...
  if CP.carFingerprint in (CAR.HONDA_ACCORD, CAR.HONDA_CIVIC_BOSCH, CAR.HONDA_CIVIC_BOSCH_DIESEL, CAR.HONDA_CRV_HYBRID, CAR.HONDA_INSIGHT,
                           CAR.ACURA_RDX_3G, CAR.ACURA_RDX_3G_MMR, CAR.HONDA_E, CAR.HONDA_CIVIC_2022, CAR.HONDA_HRV_3G):
    pass
...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    # LOW_SPEED_LOCKOUT is not worth a warning - except ACURA_RDX_3G_MMR with 70KPH error
    # NO_TORQUE_ALERT_2 can be caused by bump or steering nudge from driver
    if self.CP.carFingerprint in (CAR.ACURA_RDX_3G_MMR):
    ...
    elif self.CP.flags & HondaFlags.BOSCH_ALT_BRAKE_PCM_ACCEL:
      ret.brakePressed = cp.vl["BRAKE_MODULE"]["BRAKE_PRESSED"] != 0
    else:
