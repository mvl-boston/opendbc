...
def get_can_messages(CP, gearbox_msg):
  messages = [
    ...
    ("CAR_SPEED", 10),
    ("VSA_STATUS", 50),
    ("STEER_STATUS", 0), # TODO: figure out why required on SERIAL_STEERING
  ...
  if CP.carFingerprint in HONDA_NIDEC_HYBRID:
    messages.append(("HYBRID_STATUS", 100))
    ...
    if self.CP.carFingerprint in HONDA_BOSCH_RADARLESS:
      ...
      if self.CP.openpilotLongitudinalControl:
        if self.CP.carFingerprint in HONDA_NIDEC_HYBRID:
          ...
          ret.accFaultedNonCritical = bool(cp.vl["HYBRID_STATUS"]["BRAKE_ERROR_1"] or cp.vl["HYBRID_STATUS"]["BRAKE_ERROR_2"])
        else:
